...
if(T9_TEMPLATE_SOURCE.length!==252||T9_TEACHER_SOURCE.length!==9||!T9_TEACHER_SOURCE.some(item=>item.id==="teacher-sheet5-diem"&&item.fullName==="Cô Điểm"))throw new Error("Nguồn T9 mới nhất chưa có đủ 252 lịch và 9 giáo viên");
...
for(const day of [0,2,4])if(scheduleFor("TIGER",day,"16:00")?.teacherId!=="teacher-sheet5-han"||scheduleFor("THIÊN PHÚC",day,"17:00")?.teacherId!=="teacher-sheet5-ngan"||scheduleFor("GIA",day,"19:00")?.teacherId!=="teacher-sheet5-ngan")throw new Error("Lịch T2/T4/T6 chưa đúng màu giáo viên mới");
if(sourceStudents.has("MIE"))throw new Error("MIE đã ngừng học nhưng vẫn còn trong lịch mẫu");
...
if(scheduleFor("TUỆ MINH",5,"18:00")?.teacherId!=="teacher-sheet5-mai"||scheduleFor("MON NGUYÊN",5,"09:00")||scheduleFor("ĐÌNH BẢO",5,"16:00")?.teacherId!=="teacher-sheet5-quynh"||scheduleFor("DŨNG",5,"19:00")?.teacherId!=="teacher-sheet5-ngoc")throw new Error("Lịch gốc thứ Bảy không khớp thay đổi mới nhất");
...
if(sourceStudents.has("DÂU")||T9_TEMPLATE_SOURCE.filter(item=>item.studentKey==="THIÊN MỸ").length!==3||![1,3].every(day=>scheduleFor("THIÊN MỸ",day,"17:00")?.teacherId==="teacher-sheet5-tien")||scheduleFor("THIÊN MỸ",5,"09:00")?.teacherId!=="teacher-sheet5-tien")throw new Error("Dâu chưa được gộp đúng vào Thiên Mỹ hoặc lịch Thứ 7 09:00 Cô Tiên chưa đúng");
if(T9_TEMPLATE_SOURCE.filter(item=>item.studentKey==="DÂU TÂY").length!==6||![0,2,4].every(day=>scheduleFor("DÂU TÂY",day,"16:00")?.teacherId==="teacher-sheet5-ngan")||![1,3,5].every(day=>scheduleFor("DÂU TÂY",day,"15:00")?.teacherId==="teacher-sheet5-tien"))throw new Error("Dâu Tây chưa đủ T2–T7 theo hai khung giờ trong T9 mới nhất");
if(T9_TEMPLATE_SOURCE.filter(item=>item.studentKey==="HOÀNG KIM").length!==3||![1,3,5].every(day=>scheduleFor("HOÀNG KIM",day,"16:00")?.teacherId==="teacher-sheet5-tien"))throw new Error("Hoàng Kim chưa đúng lịch T3/T5/T7 16:00–17:00 Cô Tiên");
if(![0,2,4].every(day=>scheduleFor("PHÚC AN",day,"15:00")?.teacherId==="teacher-sheet5-quynh")||![1,3,5].every(day=>scheduleFor("KEN",day,"18:00")?.teacherId==="teacher-sheet5-quynh")||![1,3,5].every(day=>scheduleFor("NẾP",day,"18:00")?.teacherId==="teacher-sheet5-duong"))throw new Error("Màu giáo viên của Phúc An, Ken hoặc Nếp chưa khớp T9 mới nhất");
...
if(T9_MAKEUP_BALANCE_SOURCE.find(item=>item.studentKey==="VÂN MY")?.balance!==2)throw new Error("Vân My nghỉ 3 buổi, đã bù 1 buổi nên phải còn 2 buổi cần bù");
...
const invalidMorningTeacherSession={...earlySlotSession,id:"invalid-morning-teacher",studentId:student2.id,teacherId:teacher2.id};
...
export const subscribeSessions=(id,cb)=>{setTimeout(()=>cb([earlySlotSession,invalidMorningTeacherSession,session,...recurringStudentSessions,...groupedRegularPeers,sameTimeTeacher2,...filledSlotRegulars,filledSlotMakeup,lateSlotSession,maliciousSession]),0);return()=>{}};
...
export const saveTestSession=async(id,data)=>{window.__savedTestSession={id,data}};
...
export const getMaxSimultaneousStudents=()=>7;
...
export const loadWeekSchedule=async()=>[earlySlotSession,invalidMorningTeacherSession,session,...recurringStudentSessions,...groupedRegularPeers,sameTimeTeacher2,lateSlotSession];
...
export const correctStudentMakeupBalanceOnce=async()=>({updated:false,balance:2});
...
await page.waitForSelector(".daily-schedule");
if (await page.inputValue("#calendarDateFilter") !== "2026-08-14") throw new Error("Mở Lịch tuần chưa tự chọn ngày hôm nay");
if (!(await page.locator(".calendar-warning-panel").textContent()).includes("Cô Mai chỉ dạy chiều/tối")) throw new Error("Lịch tuần chưa cảnh báo đỏ khi giáo viên bị xếp sai ca");
if (!(await page.locator(".calendar-warning-panel").textContent()).includes("Cô Ngân bị trùng lịch")) throw new Error("Lịch tuần chưa cảnh báo khi một cô dạy hai bé cùng giờ");
if (await page.locator("#addTestSessionBtn").count() !== 1) throw new Error("Lịch tuần thiếu nút Thêm lịch test");
await page.click("#addTestSessionBtn");
if (await page.locator('#dialogBody select[name="studentId"], #dialogBody select[name="teacherId"]').count() !== 0) throw new Error("Lịch test không được yêu cầu học sinh hoặc giáo viên");
await page.fill('#dialogBody input[name="startTime"]',"15:00");await page.fill('#dialogBody input[name="endTime"]',"16:00");await page.click("#dialogSubmit");
await page.waitForFunction(()=>Boolean(window.__savedTestSession));
if (await page.locator("#calendarStudentSearch").count() !== 1) throw new Error("Lịch tuần thiếu tìm kiếm theo tên học sinh");
if (await page.locator("#calendarDateFilter").count() !== 1) throw new Error("Lịch tuần thiếu bộ lọc xem riêng theo ngày");
await page.selectOption("#calendarDateFilter","2026-08-10");
if (!(await page.locator("#content").textContent()).includes("Lịch ngày 10/08/2026")) throw new Error("Chọn ngày chưa chuyển lịch tuần sang chế độ xem riêng ngày đó");
if (await page.locator('.daily-schedule .daily-time-row[data-time-slot="13:00|14:00"] .daily-session-card').count() < 1) throw new Error("Lịch ngày chưa rút gọn học sinh thành các ô sát khung giờ");
if (await page.locator('[data-session="session-2"]').count()) throw new Error("Bộ lọc ngày vẫn hiển thị buổi của ngày khác");
await page.selectOption("#calendarDateFilter","");
...
await page.click("#newWeek");
await page.waitForSelector('#formDialog[open] input[name="startDate"]');
if (await page.locator('input[name="startDate"]').inputValue() !== "2026-08-17") throw new Error("Tạo tuần tiếp theo chưa mặc định vào Thứ Hai sau tuần cuối cùng");
...
if (!(await page.locator("#dialogBody").textContent()).includes("7 phòng") || !(await page.locator("#dialogBody").textContent()).includes("4 cô") || await page.locator(".weekly-vacancy-list>details").count() < 1) throw new Error("Xem nhanh lịch tuần chưa tách đúng 7 phòng và 4 cô buổi sáng");
await page.click("#dialogCancel");
...
await page.waitForSelector("#formDialog[open]");
if (await page.locator(".direct-makeup-teacher").count() !== 1) throw new Error("Danh sách nút giáo viên đang rảnh không đúng");
if (await page.locator('select[name="teacherId"]').count() !== 0) throw new Error("Form học bù vẫn còn dropdown chọn giáo viên");
if (!(await page.locator("#dialogBody").textContent()).includes("Còn 2/7 chỗ")) throw new Error("Form học bù chưa hiển thị đúng giới hạn 7 học sinh");
await page.click("#dialogCancel");
...
await page.click("#dialogSubmit");
await page.waitForFunction(()=>document.querySelector("#dialogTitle")?.textContent.includes("Phương án dạy thay"));
...
if (await page.locator('.teacher-replacement-row select option[value="student_off"]').count() !== 1) throw new Error("Phương án nghỉ phép thiếu lựa chọn cho học sinh nghỉ và cộng buổi bù");
await page.click("#dialogCancel");
await page.click("#studentProfileClose");
...
if (await page.locator('[data-report-tab="students"]').getAttribute("aria-selected") !== "true" || await page.locator('[data-report-panel="students"]').isHidden()) throw new Error("Báo cáo tháng chưa ưu tiên tab Học sinh ở vị trí đầu");
const studentHeaders=(await page.locator('[data-report-panel="students"] thead th').allTextContents()).map(text=>text.trim());
if (studentHeaders.some(text=>text.includes("Giờ")) || !studentHeaders.includes("Buổi học mặc định") || !studentHeaders.includes("Tổng buổi thực tế")) throw new Error("Báo cáo học sinh chưa thống nhất toàn bộ số liệu theo buổi");
...
await page.click('[data-report-tab="teachers"]');
if (await page.locator('[data-report-tab="teachers"]').getAttribute("aria-selected") !== "true" || await page.locator('[data-report-panel="students"]').isVisible()) throw new Error("Chuyển tab Giáo viên chưa ẩn riêng báo cáo Học sinh");
...
const reportTabOrder=await page.locator("[data-report-tab]").allTextContents();
if (!reportTabOrder[0].includes("Học sinh") || !reportTabOrder[1].includes("Giáo viên")) throw new Error("Thứ tự tab báo cáo tháng chưa đặt Học sinh trước Giáo viên");
...
await page.waitForSelector(".daily-schedule");
const mobileDailyLayout=await page.locator(".daily-schedule").evaluate(root=>({overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,width:root.getBoundingClientRect().width,cards:[...root.querySelectorAll(".daily-session-card")].every(card=>card.getBoundingClientRect().right<=root.getBoundingClientRect().right+1)}));
if (mobileDailyLayout.overflow>1 || !mobileDailyLayout.cards) throw new Error("Lịch ngày rút gọn vẫn bị tràn ngang trên điện thoại");
await page.selectOption("#calendarDateFilter","");
